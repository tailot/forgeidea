 ...
 * @type {string}
 */
var outputJsonFileName = './src/static-assets/env.json';
/**
 * Absolute path to the output JSON file.
 * @type {string}
 */
var outputJsonFilePath = path.resolve(__dirname, outputJsonFileName);
/**
 * Filename for the temporary version of the output JSON file.
 * Used for atomic write operations.
 * @type {string}
 */
var tempOutputJsonFileName = "".concat(outputJsonFileName, ".tmp");
/**
 * Absolute path to the temporary output JSON file.
 * @type {string}
 */
var tempOutputJsonFilePath = path.resolve(__dirname, tempOutputJsonFileName);
console.log("Reading keys from: ".concat(prodEnvFilePath));
/**
 ...
 */
var envObjectMatch = inputFileContent.match(/export const environment\s*=\s*\{([\s\S]*?)\};/);
// Extract keys from the environment object in the input file
if (!envObjectMatch || !envObjectMatch[1]) {
    console.error("Error: Could not find the 'export const environment = {...};' object in the file \"".concat(prodEnvFileName, "\"."));
 ...
 * @type {string}
 */
var objectContent = envObjectMatch[1];
var propertyRegex = /^\s*(['"]?)([a-zA-Z_$][0-9a-zA-Z_$]*)\1\s*:/gm; // g = globale, m = multiline
var match;
/**
...
if (keys.length === 0) {
    console.warn("Warning: No valid keys found in the 'environment' object in \"".concat(prodEnvFileName, "\". Both output files will represent an empty environment object."));
}
else {
    console.log("Keys extracted from the source file: ".concat(keys.join(', ')));
}
/**
 ...
 * @type {string}
 */
var outputTsString = 'export const environment = {\n';
for (var i = 0; i < keys.length; i++) {
    var key = keys[i];
    var envValue = process.env[key];
    // Logic for JSON
    if (envValue === undefined) {
        console.warn("Warning: Environment variable \"".concat(key, "\" not found in process.env. Setting \"").concat(key, "\" to null in the JSON output."));
    ...
    var formattedValueTs = void 0;
    if (envValue === undefined) {
        console.warn("Warning: Environment variable \"".concat(key, "\" not found in process.env. Setting \"").concat(key, "\" to undefined in ").concat(prodEnvFileName, "."));
        ...
        var escapedValue = envValue
            .replace(/\\/g, '\\\\') // Escape backslashes first!
            .replace(/'/g, "\\'") // Escape single quotes
            .replace(/\n/g, '\\n') // Escape newlines
            .replace(/\r/g, '\\r'); // Escape carriage returns
        formattedValueTs = "'".concat(escapedValue, "'");
    }
    outputTsString += "  ".concat(key, ": ").concat(formattedValueTs).concat(i < keys.length - 1 ? ',' : '', "\n");
}
outputTsString += '};\n';
/**
 ...
 * @type {string}
 */
var outputJsonString = JSON.stringify(outputEnvData, null, 2); // Format JSON with 2 spaces indentation
// Write to static-assets/env.json
try {
    console.log("Writing JSON content to: \"".concat(outputJsonFilePath, "\""));
    ...
    console.log("Content temporarily written to: \"".concat(tempOutputJsonFileName, "\""));
    fs.renameSync(tempOutputJsonFilePath, outputJsonFilePath);
    console.log("File \"".concat(outputJsonFileName, "\" updated successfully."));
}
catch (error) {
    console.error("Error writing ".concat(outputJsonFileName, ":"), error);
    if (fs.existsSync(tempOutputJsonFilePath)) {
        try {
            fs.unlinkSync(tempOutputJsonFilePath);
            console.error("Cleaned up temporary file: \"".concat(tempOutputJsonFileName, "\"."));
        }
        catch (e) {
            console.error("Error cleaning up temporary file \"".concat(tempOutputJsonFileName, "\""));
        }
    }
    process.exit(1);
}
...
try {
    console.log("Rewriting TypeScript environment file: \"".concat(prodEnvFilePath, "\""));
    fs.writeFileSync(tempProdEnvFilePath, outputTsString, 'utf8');
    console.log("Content temporarily written to: \"".concat(tempProdEnvFileName, "\""));
    fs.renameSync(tempProdEnvFilePath, prodEnvFilePath);
    console.log("File \"".concat(prodEnvFileName, "\" updated successfully."));
}
catch (error) {
    console.error("Error writing ".concat(prodEnvFileName, ":"), error);
    if (fs.existsSync(tempProdEnvFilePath)) {
        try {
            fs.unlinkSync(tempProdEnvFilePath);
            console.error("Cleaned up temporary file: \"".concat(tempProdEnvFileName, "\"."));
        }
        catch (e) {
            console.error("Error cleaning up temporary file \"".concat(tempProdEnvFileName, "\""));
        }
    }
    process.exit(1);
}
